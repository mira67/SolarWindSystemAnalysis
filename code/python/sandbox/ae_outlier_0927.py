#Autoencoder for anomaly detection test
...
def get_signal(size, outliers_size=0.01):
    ...
    #adding noise
    sig[random_indices] = np.random.randint(6, 8, 1)[0]
...
def main():
    ...
    # no outliers.
    signal_train, _ = get_signal(signal_length, outliers_size=0)
    
    x_train, _ = prepare_sequences(signal_train, window_length, [])

    # 1 percent are outliers.
    signal_test, random_indices = get_signal(signal_length, outliers_size=0.01)
    ...
    if new_model == 0:#os.path.isfile(model_file):
