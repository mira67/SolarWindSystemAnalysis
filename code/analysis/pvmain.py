...
dateName = '#data_date'
timeRg = ['08:00','16:00'];#maybe calculate based on the sunshine value, contextual information
...
def faultDetection(fullname):
    ...
    #day result array
    faultArr = np.zeros((numDays,stringNum))
    dayCount = 0
    ...
    for dt in dateList:
        ...
        dayFaultCount = np.zeros((1,stringNum));
        
        try:#excetpion catch is necessary for robust code
            for t in range(lenT):
                
                # grab strings I from each timestamp
                x=dfday.iloc[0,1:stringNum+1].as_matrix().reshape(stringNum,1)
    ...
    outPath = 'E:/myprojects/solarwind/data/stage1_2/'
    hlx = os.path.basename(fullname)
    dayResults.to_csv(outPath+hlx)

    return 'Success!'
    # after a day's process, raise alarm based on total timing
    #method 1: threshold
    #method 2: clustering
    # summarize reulsts, print daily fault strings to file

if __name__ == '__main__':
    inPath = 'E:/myprojects/solarwind/data/test/'
